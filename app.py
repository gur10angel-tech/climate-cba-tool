...
Ask all questions in one message."""
        ...
        # ── Stage: user provides data → build analysis ─────────────────────────
            ...
            # Query KB for cost/benefit literature context
...
Use the following figures to populate formula parameters (VSL, mortality factors, efficiency rates, costs).
For each value taken from literature, cite the source in the corresponding _source field.
...
---"""
            ...
            # ── Auto-retry if JSON extraction failed ─────────────────────────
            if data is None:
                with st.spinner("Refining output format..."):
                    retry_messages = history + [
                        ...
                        {"role": "user", "content":
                            ...
                            "Please output ONLY the JSON object — no explanatory text, "
                            "no markdown fences, no preamble. Start your response with { and end with }."}
                    ]
            ...
            if data:
                ...
                # Build sources summary from benefit_components
                sources_used = set()
                ...
                sources_note = ""
                if sources_used:
                    sources_note = "\n\n📚 **Literature sources used:** " + " | ".join(list(sources_used)[:4])
                st.session_state.messages.append({
                    "role": "assistant",
                    "content": (
                        f"✅ Analysis complete for **{data['problem_title']}**. "
                        f"I've analyzed {len(data['measures'])} measure(s){specialist_note}. "
                        f"Each benefit is computed via a live Excel formula — every parameter "
                        f"shows its source.{sources_note}\n\n"
                ...
                st.session_state.messages.append({
                    "role": "assistant",
                    "content": (
                        "I wasn't able to generate a valid analysis JSON. "
                        "Please try rephrasing your inputs, or type **'use defaults'** "
                        "and I'll fill everything from literature."
                    )
                })
            st.rerun()
