...
def query_bedrock_kb(query: str) -> str:
    ...
    try:
        ...
        # Log to session state for debug panel
        if "kb_debug_log" not in st.session_state:
            st.session_state.kb_debug_log = []
        st.session_state.kb_debug_log.append({
            "query": query[:120],
            "n_chunks": len(chunks),
...
st.set_page_config(page_title="CBAi — Climate Adaptation CBA", page_icon="🌍", layout="wide")
...
.landing-hero {
    ...
    background: linear-gradient(180deg, #f0fdf4 0%, #ffffff 85%);
    border: 1px solid #d1fae5;
    border-radius: 16px;
}
.hero-eyebrow {
...
.cap-card {
    ...
    color: #14532d;
    line-height: 1.5;
    box-shadow: 0 1px 2px rgba(5, 46, 22, 0.05);
...
""")
    ...
    with st.expander("About", expanded=False):
        st.markdown("""
**CBAi — Climate Benefit-Cost Intelligence**
...
""")
    ...
    # Dev-only panel — visible only when the URL includes ?debug=1
    if st.query_params.get("debug"):
        with st.expander("🔍 KB Debug Log", expanded=False):
            kb_log = st.session_state.get("kb_debug_log", [])
            if kb_log:
                for i, entry in enumerate(kb_log):
                    st.markdown(f"**Query {i+1}:** `{entry['query']}`")
                    st.caption(f"Chunks returned: {entry['n_chunks']} · Sources: {', '.join(entry['sources'][:5])}")
                if st.button("Clear log", key="clear_kb_log"):
                    st.session_state.kb_debug_log = []
                    st.rerun()
            else:
                st.caption("No KB queries yet — start an analysis to see results.")
...
STANDARD TYPES (use when applicable):
  "avoided_mortality", "energy_savings", "morbidity_savings", "property_value_uplift"
ADDITIONAL BENEFITS FROM LITERATURE:
  Use "generic_annual" for any benefit identified in the academic literature passages below
  that does not fit the standard types. This includes but is not limited to:
  - reduced workplace absenteeism / productivity losses
  - avoided infrastructure damage / maintenance costs
  - tourism or recreation value
  - water treatment cost savings / stormwater management
  - carbon sequestration / emissions reduction
  - air quality improvement / PM2.5 reduction
  - biodiversity / ecosystem services
  - noise reduction
  - reduced healthcare utilization beyond mortality/morbidity
  For "generic_annual", provide:
    "name": descriptive name of this benefit,
    "value": annual benefit in currency millions,
    "value_source": MANDATORY — must include ALL of the following:
      (a) The specific paper or source (Author Year, Table/Page if possible)
      (b) The calculation or formula used to derive the value
      (c) The key input assumptions
      Example: "Estrada et al. 2017 Table S2: 1.50 EUR/m²/yr × 1,000,000 m² green roof area = 1.5 EUR M/yr"
      Example: "Getter et al. 2009: 0.5 kg CO₂/m²/yr × 1,000,000 m² × 90 EUR/tCO₂ ÷ 1000 = 0.045 EUR M/yr"
      NEVER write just "Literature estimate" — this is not acceptable.
      If you cannot cite a specific paper and calculation, do NOT include the benefit.
  IMPORTANT: Actively search the literature passages for ALL monetisable benefits —
  do not stop at 2-3. A thorough CBA should capture every benefit the literature supports.
...
Every numeric parameter needs a "_source" field with a SPECIFIC citation:
  GOOD: "vsl_source": "OECD 2012 ENV/WKP(2012)3, Table 3.1 — EU median adjusted to Israel PPP (×0.89)"
  GOOD: "value_source": "Getter et al. 2009 Table 2: 0.5 kg CO₂/m²/yr × 1,000,000 m² × 90 EUR/tCO₂ / 1000 = 0.045 EUR M/yr"
  BAD:  "value_source": "Literature estimate"  ← NEVER DO THIS
  BAD:  "value_source": "Expert estimate"  ← NEVER DO THIS
  If you cannot cite a specific source, use the knowledge base passages provided below.
  If the knowledge base does not contain the value either, state "No source available — estimate based on [reasoning]".
...
CITATION RULE: Every "_source" or "data_source" field MUST include: (a) Author Year, (b) specific table/page/section, (c) the calculation showing how the value was derived. NEVER write just "Literature estimate" or "Expert estimate"."""
...
# ── Connection settings (sidebar, below Settings sections) ─────────────────────
_kb_from_server = bool(_secret("BEDROCK_KB_ID"))
with st.sidebar:
    with st.expander("🔑 API Connection", expanded=not st.session_state.api_key):
...
if not st.session_state.api_key:
    st.caption("🔑 To run an analysis, add your Anthropic API key under **API Connection** in the sidebar.")
...
<div class="landing-hero">
  <div class="hero-eyebrow">Climate Economics Lab</div>
  <h2>CBAi &mdash; Climate Benefit-Cost Intelligence</h2>
...
if send and user_input.strip() and not st.session_state.api_key:
    st.warning("🔑 Add your Anthropic API key in the sidebar (**API Connection**) to start the analysis.")
if send and user_input.strip() and st.session_state.api_key:
        ...
        if st.session_state.stage == "problem":
            ...
            with st.spinner("Searching academic literature for relevant adaptation measures..."):
                ...
                # Run 4 targeted KB queries to find measures from the literature
                ...
                kb_cobenefits = query_bedrock_kb(
                    f"co-benefits economic benefits monetization energy savings property value productivity {user_input}"
                )
...
2. Propose 3-5 concrete adaptation measures for this problem.
   ...
   - **ALL benefit categories** mentioned in the papers for this measure — be exhaustive.
     Include both direct benefits (avoided mortality, energy savings, morbidity reduction)
     AND co-benefits / indirect benefits (carbon sequestration, property value uplift,
     reduced stormwater runoff, air quality improvement, productivity gains, reduced
     infrastructure maintenance, biodiversity, noise reduction, tourism value, etc.)
     List every benefit the literature supports, even if small.
...
---"""
            ...
            # Get the measure names from the conversation for targeted queries
            _prev_assistant = next((m["content"] for m in reversed(st.session_state.messages)
                                    if m["role"] == "assistant"), "")

            kb_benefits = query_bedrock_kb(
                f"benefit types categories co-benefits monetization urban heat adaptation {user_input}"
            )
            kb_formulas = query_bedrock_kb(
                f"cost benefit calculation formula methodology NPV BCR discount rate {st.session_state.problem_text}"
            )
            kb_params = query_bedrock_kb(
                f"VSL mortality rate heat reduction efficiency cost per unit parameter value {st.session_state.problem_text}"
            )

            kb_data_section = ""
            for label, kb_text in [
                ("BENEFIT TYPES & CATEGORIES FROM LITERATURE", kb_benefits),
                ("CALCULATION FORMULAS & METHODOLOGY FROM LITERATURE", kb_formulas),
                ("PARAMETER VALUES FROM LITERATURE", kb_params),
            ]:
                if kb_text and not kb_text.startswith("[KB unavailable"):
                    kb_data_section += f"\n\n--- {label} ---\n{_trunc(kb_text, 5000)}"
            kb_data_section = _trunc(kb_data_section, 14000)

            if kb_data_section:
                prompt_text += f"""

═══════════════════════════════════════════════════════════════
ACADEMIC LITERATURE — RETRIEVED FROM KNOWLEDGE BASE
═══════════════════════════════════════════════════════════════
The following passages come directly from peer-reviewed papers indexed in the knowledge base.
You MUST use them to:

1. IDENTIFY ALL BENEFIT TYPES mentioned in the papers for these measures — do NOT limit yourself
   to the standard types listed above. If the literature mentions benefits like "reduced workplace
   absenteeism", "tourism revenue", "reduced water treatment costs", "improved mental health",
   "reduced pavement maintenance", etc. — include them using "generic_annual" type with the
   formula and value from the paper.

2. USE THE FORMULAS from the papers when they differ from the defaults above. If a paper provides
   a specific calculation method (e.g. a different VSL derivation, a different dose-response
   function, a regional cost factor), prefer the paper's formula and cite it.

3. USE THE PARAMETER VALUES from the papers. If a paper provides a specific value for a parameter
   (e.g. heat_mortality_factor = 0.042 from a Mediterranean study), prefer it over the defaults
   above and cite the paper in the _source field.

4. For each benefit component, the _source field MUST reference the specific paper it came from
   (not just "literature estimate"). For "generic_annual" benefits, the "value_source" MUST include:
   - The paper name (Author Year)
   - The specific table, page, or section where the value appears
   - The CALCULATION showing how you derived the annual value
   Example: "Johnson et al. 2020 Table 5: 102 patient-days/death × 650 EUR/day × 30.5 deaths = 2.01 EUR M/yr"
   Example: "Estrada et al. 2017 Supp. Table 2: stormwater retention 1.50 EUR/m²/yr × 1,000,000 m² = 1.5 EUR M/yr"
   If you cannot trace a value to a specific paper and show the calculation, do NOT include it.

{kb_data_section}

═══════════════════════════════════════════════════════════════
END OF LITERATURE SECTION
═══════════════════════════════════════════════════════════════
...
NOTE: No academic literature was retrieved from the knowledge base for this query.
Use the default parameter values specified above and cite their standard sources.
