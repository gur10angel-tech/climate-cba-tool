...
def extract_methodology_from_kb(measures_text: str, problem_text: str) -> str:
    """
    Run 4 targeted KB queries to extract methodology for selected measures.
    Returns combined literature passages relevant to formulas, costs, and effectiveness.
    """
    kb_id      = st.session_state.get("kb_id", "").strip()
    aws_key    = st.session_state.get("aws_key", "").strip()
    aws_secret = st.session_state.get("aws_secret", "").strip()
    ...
    queries = [
        f"benefit calculation formula VSL mortality heat wave {measures_text}",
        f"cost per unit implementation {measures_text} urban heat island EUR USD",
        f"mortality reduction effectiveness deaths prevented cooling {measures_text}",
        f"discount rate social CBA public health infrastructure methodology",
    ]

    all_chunks = []
    try:
        client_kb = boto3.client(
            "bedrock-agent-runtime",
            region_name=region,
            aws_access_key_id=aws_key,
            aws_secret_access_key=aws_secret,
...
CRITICAL RULES:
- Every measure MUST have its own distinct "annual_benefit" value reflecting its specific benefit stream.
- Do NOT give all measures the same annual_benefit — each measure has different effectiveness and scale.
- Base annual_benefit on literature estimates for that specific measure type (e.g. early warning system benefits differ greatly from infrastructure benefits).
- annual_benefit must be realistic relative to capex (BCR typically 0.5–10 for climate adaptation measures).
...
Keep your response concise and practical."""
        ...
        # ── Stage: user selects measures ───────────────────────────────────────
            ...
            with st.spinner("Reading academic literature for selected measures..."):

                # Extract methodology from KB using 4 targeted queries
                kb_methodology = extract_methodology_from_kb(user_input, st.session_state.problem_text)

                kb_section = ""
                if kb_methodology and not kb_methodology.startswith("[KB unavailable"):
                    kb_section = f"""
The following passages were retrieved from peer-reviewed academic papers.
They contain formulas, cost ranges, effectiveness values, and methodological guidance.
Use them to:
  (a) understand HOW to calculate each benefit (formulas, multipliers)
  (b) identify WHICH local data points you need from the user
  (c) identify WHICH values you can take directly from the literature (note the source)
...
{kb_section}
Based on the academic literature above (and your own knowledge where literature is silent),
do the following:

1. For each selected measure, briefly explain HOW the main benefits will be calculated
   (e.g. "Avoided mortality = VSL × deaths_avoided, where deaths_avoided = population × mortality_rate × heat_reduction_efficiency").
   Cite the source paper where possible.

2. List the data clearly in two groups:
   **Local data needed from user:** e.g. population size, project area, local costs
   **Literature defaults I will use:** e.g. VSL, BCR ranges, effectiveness rates — list the value and source

3. Ask the user ONLY for the local data. Be specific about units. Keep the list short and grouped by measure."""
                })
        ...
        # ── Stage: user provides data → build analysis ─────────────────────────
            ...
            # Re-query KB with fresh targeted queries based on what user provided
            kb_methodology = extract_methodology_from_kb(
                st.session_state.problem_text, user_input
            )
            kb_addition = ""
            if kb_methodology and not kb_methodology.startswith("[KB unavailable"):
...
PEER-REVIEWED LITERATURE (use to fill any gaps the user did not provide):
For every value you take from the literature below, note the source paper in the
"data_source" field of the relevant measure. If you use a default, write
"Literature default — [Author, Year]".
...
---"""
            ...
            with st.spinner("Building analysis from your data + academic literature..."):
                history = [{"role": m["role"], "content": m["content"]} for m in st.session_state.messages]
            ...
            try:
                ...
                # Build a summary of literature sources used
                sources_used = []
                for m in data.get("measures", []):
                    ds = m.get("data_source", "")
                    if ds and ds not in sources_used:
                        sources_used.append(ds)
                ...
                sources_note = ""
                if sources_used:
                    sources_note = "\n\n📚 **Literature sources used:** " + " | ".join(sources_used[:4])

                st.session_state.messages.append({
                    "role": "assistant",
                    "content": f"✅ Analysis complete for **{data['problem_title']}**. I've analyzed {len(data['measures'])} measure(s){specialist_note}. Values not provided by you were drawn from peer-reviewed literature and are noted in the Excel file.{sources_note}\n\nClick **Download Excel** below to get your full CBA model."
                })
