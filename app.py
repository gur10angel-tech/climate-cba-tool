...
You are an expert in climate adaptation CBA. Extract all data and produce a JSON object.
Respond ONLY with valid JSON, no markdown fences.

CRITICAL ARCHITECTURE RULE:
Every benefit component MUST be expressed as a structured formula with individual parameters —
NOT as a single pre-computed number. Excel will compute the final value from these parameters,
so each parameter must be its own field with a value and a source.

Supported formula types and their required parameters:

TYPE "avoided_mortality":
  population_at_risk      — number of people exposed (local data or literature estimate)
  mortality_rate          — annual deaths per person in at-risk group (e.g. 0.010 = 1%)
  heat_mortality_factor   — excess deaths per person per CDD (literature: ~0.00083 for Mediterranean)
  heat_reduction_efficiency — fraction of heat risk eliminated by this measure (0.0–1.0)
  vsl                     — Value of Statistical Life in currency units (e.g. 3800000 for EUR)
  Formula: population × mortality_rate × heat_mortality_factor × heat_reduction_efficiency × vsl

TYPE "energy_savings":
  area_m2                 — project area in square meters
  energy_reduction_kwh_m2 — kWh saved per m² per year (literature: 15–45 for cool roofs)
  electricity_tariff      — local price per kWh in project currency (e.g. 0.14 EUR/kWh)
  Formula: area_m2 × energy_reduction_kwh_m2 × electricity_tariff / 1,000,000 (→ millions)

TYPE "morbidity_savings":
  cases_avoided_per_year  — heat-attributable hospitalizations avoided
  hospitalization_cost    — cost per hospital day in project currency
  avg_length_of_stay_days — average hospital stay in days (literature: 5.2 days)
  Formula: cases_avoided × hosp_cost × avg_los / 1,000,000

TYPE "property_value_uplift":
  affected_area_m2        — area of properties benefiting
  property_value_per_m2   — average property value per m² in project currency
  uplift_fraction         — fractional increase in property value (literature: 0.02–0.05)
  Formula: area × value_per_m2 × uplift_fraction / 1,000,000

TYPE "generic_annual":  ← use ONLY when no formula type fits
  annual_value            — pre-computed annual benefit in currency millions
  Use this as a last resort. Always prefer a formula type above.

For EACH formula parameter, add a "_source" field naming the paper/source:
  e.g. "heat_mortality_factor": 0.00083,
       "heat_mortality_factor_source": "Gasparrini et al. 2017, Lancet Table 3 — Mediterranean cluster"

{
  ...
  "measures": [
    {
      ...
      "capex": 10.0,
      "capex_source": "User provided / Literature estimate — Author (Year)",
      "annual_opex": 0.5,
      "opex_source": "User provided / Literature estimate — Author (Year)",
      "co_benefits": "Non-monetary benefits not included in BCR",
      "lifetime_years": 30,
      "feasibility": "High|Medium|Low",
      "uncertainty": "Low|Medium|High",
      "benefit_components": [
        {
          "name": "Avoided Heat Mortality",
          "type": "avoided_mortality",
          "population_at_risk": 130000,
          "population_at_risk_source": "User provided / Israel CBS 2023",
          "mortality_rate": 0.010,
          "mortality_rate_source": "Israel Ministry of Health, age-standardised rate 65+",
          "heat_mortality_factor": 0.00083,
          "heat_mortality_factor_source": "Gasparrini et al. 2017, Lancet Table 3 — Mediterranean cluster",
          "heat_reduction_efficiency": 0.28,
          "heat_reduction_efficiency_source": "Bouchama et al. 2007, NEJM — AC efficacy in heatwave mortality",
          "vsl": 3800000,
          "vsl_source": "OECD 2012 ENV/WKP meta-analysis median, EUR-adjusted"
        },
        {
          "name": "Energy Savings",
          "type": "energy_savings",
          "area_m2": 500000,
          "area_m2_source": "User provided — project scope",
          "energy_reduction_kwh_m2": 25,
          "energy_reduction_kwh_m2_source": "Zinzi & Fasano 2009 — cool roof energy savings Mediterranean climate",
          "electricity_tariff": 0.14,
          "electricity_tariff_source": "Israeli Electricity Authority 2024 residential tariff"
        }
      ]
    }
  ],
  ...
  "key_assumptions": [
    {"text": "Assumption text", "source": "Author (Year), specific table/page"}
  ],
  "data_gaps": "What local data would most improve this analysis"
}

RULES:
- Use "avoided_mortality", "energy_savings", "morbidity_savings", "property_value_uplift" wherever possible.
- "generic_annual" is a last resort — only if no formula type applies.
- Every numeric parameter MUST have a corresponding "_source" field with author, year, and table/page if available.
- CAPEX and OPEX must also have source fields.
- BCR should realistically fall between 0.5 and 10 for climate adaptation measures.
- Each measure must have at least one benefit_component.
- Use the user's local data (population, area, costs) for endogenous parameters.
- Use literature values for mortality rates, efficiency factors, VSL, unit costs."""
    ...
    if send and user_input.strip() and st.session_state.api_key:
        ...
        if st.session_state.stage == "problem":
            ...
            with st.spinner("Analyzing problem..."):
                # Query KB if connected — targeted to measure proposal stage
                kb_context = query_bedrock_kb(
                    f"urban heat island adaptation measures interventions {user_input}"
                )
                kb_section = ""
                if kb_context and not kb_context.startswith("[KB unavailable"):
                    kb_section = f"""
The following passages were retrieved from peer-reviewed academic papers in the knowledge base.
Use them to inform your measure suggestions and cost ranges:
...
Ask all questions in one message."""
        ...
        # ── Stage: user provides data → build analysis ─────────────────────────
            ...
            # Query KB for cost/benefit figures — targeted to CBA quantification
            kb_cba = query_bedrock_kb(
                f"cost benefit NPV BCR cost per unit benefit value {st.session_state.problem_text}"
            )
            if kb_cba and not kb_cba.startswith("[KB unavailable"):
                kb_addition = f"""

ADDITIONAL CONTEXT FROM PEER-REVIEWED LITERATURE:
Use the following figures from academic papers to ground your cost and benefit estimates.
Prefer these values over generic defaults where relevant:
