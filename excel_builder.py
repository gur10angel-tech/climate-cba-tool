...
def _inputs(wb, data, am=None):
    ...
    if sub_total_ref:
        ...
        # Generic mode — each measure has its own annual_benefit from the JSON (exogenous literature value)
        for ci, m in enumerate(measures, 2):
            benefit_val = m.get("annual_benefit") or 0
            _exog_cell(ws, row, ci, benefit_val, fmt="#,##0.0")
        _cell(ws, row, n+2, "🟠 Exogenous — literature-sourced estimate (editable)")
    row += 1
