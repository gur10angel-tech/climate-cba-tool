...
def _inputs(wb, data, am=None):
    ...
    if sub_total_ref:
        ...
        # Generic mode — each measure has its own annual_benefit from the JSON
        for ci, m in enumerate(measures, 2):
            benefit_val = m.get("annual_benefit") or 0
            c = ws.cell(row=row, column=ci, value=benefit_val)
            c.font = Font(name="Arial", bold=True, color=BLUE, size=10)
            c.number_format = "#,##0.0"; c.border = _bd()
            c.fill = PatternFill("solid", fgColor=C_LIGHT)
            c.alignment = Alignment(horizontal="right")
        _cell(ws, row, n+2, "Blue = editable input  |  Each measure has its own benefit estimate")
    row += 1
