...
def build_excel(data: dict, path: str, assumptions_override: dict = None):
    ...
    _summary(wb, data, rm)
    if any(m.get("benefit_components") or m.get("benefit_types") for m in data.get("measures", [])):
...
def _inputs(wb, data, am=None):
    ...
    if sub_total_ref:
        ...
        for ci, m in enumerate(measures, 2):
            # Prefer annual_benefit if present; otherwise sum benefit_components
            benefit_val = m.get("annual_benefit")
            if benefit_val is None:
                comps = m.get("benefit_components") or []
                if isinstance(comps, list):
                    benefit_val = sum(
                        (c.get("annual_value") or c.get("value") or 0)
                        for c in comps
                        if c.get("type") == "generic_annual"
                    )
                elif isinstance(comps, dict):
                    benefit_val = sum(v for v in comps.values() if isinstance(v, (int, float)))
            benefit_val = benefit_val or 0
            _exog_cell(ws, row, ci, benefit_val, fmt="#,##0.0")
        has_formulas = any(
            (m.get("benefit_components") and not isinstance(m.get("benefit_components"), dict))
            for m in measures
        )
...
def _benefit_detail(wb, data, rm):
    ...
    def _build_avoided_mortality(comp, start_row):
        ...
        r_hmf   = r; _prow(r, "Heat-Mortality Factor (excess deaths/CDD)", hmf, "deaths/person/CDD", "", comp.get("heat_mortality_factor_source","Gasparrini et al. 2017"), is_exog=True, fmt="0.000000"); r+=1
    ...
    def _build_property_value_uplift(comp, start_row):
        ...
        formula = f"=B{r_area}*B{r_val}*B{r_upl}/1000000"
        cell = _result(r, comp.get("name","Property Value Uplift"), formula,
                       "Area × Value/m² × Uplift Fraction ÷ 1,000,000"); r+=1
        return cell.coordinate, r
