...
def _sanitise_mortality_params(data: dict) -> list[str]:
    """
    Validate and auto-correct avoided_mortality parameters before writing to Excel.
    Returns a list of warning strings (empty if all OK).
    
    Key correction: if heat_mortality_factor > 0.15 it is almost certainly wrong.
    Gasparrini 2017 (Mediterranean) gives 2-5%; Hyyrynen 2025 uses 6.7% for Helsinki.
    Values above 15% are methodologically unsound for annual attributable fractions.
    """
    warnings = []
    for m in data.get("measures", []):
        for comp in (m.get("benefit_components") or []):
            ...
            mname = m.get("name", "measure")

            # VSL: auto-convert if expressed in millions
            vsl = comp.get("vsl", 0)
            if isinstance(vsl, (int, float)) and 0 < vsl < 1000:
                comp["vsl"] = int(vsl * 1_000_000)
                warnings.append(
                    f"[AUTO-FIX] {mname} / {name}: vsl {vsl} → {comp['vsl']} (converted from millions to units)"
                )

            # heat_mortality_factor: cap at 0.10 with warning
            # Literature range for Mediterranean/Middle-East cities: 0.02–0.07
            # Hyyrynen 2025 (Helsinki, hot summers): 0.067 (6.7%)
            # Gasparrini 2017 (Mediterranean median): ~0.035 (3.5%)
            # Values above 0.10 (10%) are not supported by peer-reviewed literature
            hmf = comp.get("heat_mortality_factor", 0)
            if isinstance(hmf, (int, float)) and hmf > 0.10:
                old_hmf = hmf
                comp["heat_mortality_factor"] = 0.067  # Hyyrynen 2025 conservative default
                comp["heat_mortality_factor_source"] = (
                    comp.get("heat_mortality_factor_source", "") +
                    f" [AUTO-CORRECTED from {old_hmf:.3f} to 0.067 — "
                    "exceeds peer-reviewed cap of 0.10; "
                    "Gasparrini 2017 Mediterranean = 0.035, Hyyrynen 2025 = 0.067]"
                )
                warnings.append(
                    f"[AUTO-FIX] {mname} / {name}: heat_mortality_factor {old_hmf:.3f} → 0.067 "
                    f"(exceeded cap 0.10; literature range 0.02–0.07 for Mediterranean cities)"
                )

            # Cap heat_reduction_efficiency at 0.85
            # No climate adaptation measure achieves >85% heat death reduction in practice.
            # Bouchama 2007: home AC ~77%. WHO 2013: best-case infrastructure ~80-85%.
            eff = comp.get("heat_reduction_efficiency", 0)
            if isinstance(eff, (int, float)) and eff > 0.85:
                old_eff = eff
                comp["heat_reduction_efficiency"] = 0.77
                comp["heat_reduction_efficiency_source"] = (
                    comp.get("heat_reduction_efficiency_source", "") +
                    f" [AUTO-CORRECTED from {old_eff:.2f} to 0.77 — "
                    "no single measure achieves >85% heat death reduction; "
                    "Bouchama 2007 NEJM: home AC = 0.77]"
                )
                warnings.append(
                    f"[AUTO-FIX] {mname} / {name}: heat_reduction_efficiency {old_eff:.2f} → 0.77 "
                    "(capped; literature max ~0.85 for best-case AC scenarios)"
                )

            # Sanity check: compute implied deaths_avoided
            pop  = comp.get("population_at_risk", 0)
            mr   = comp.get("mortality_rate", 0)
            hmf2 = comp.get("heat_mortality_factor", 0)
            eff2 = comp.get("heat_reduction_efficiency", 0)
            deaths = pop * mr * hmf2 * eff2
            if deaths > 300:
                warnings.append(
                    f"[WARNING] {mname} / {name}: implied deaths_avoided = {deaths:.0f}/yr "
                    f"(pop={pop}, mort={mr:.4f}, hmf={hmf2:.3f}, eff={eff2:.2f}). "
                    "Exceeds 300/yr — verify population_at_risk is only the sub-group served by this measure."
                )
...
def _sensitivity(wb, data, rm):
    ...
    for sv in sens_vars:
        ...
        is_ben_scaler = any(k in sv_key_note for k in (
            "vsl", "value of statistical life", "heat mortality",
            "electricity tariff", "tariff", "population", "vulnerable population",
            "mortality rate", "hospitalization", "benefit multiplier",
        ))
        if is_ben_scaler:
            note_cell = ws.cell(row=row, column=1,
                value=f"ℹ️ Multiplier applied to total annual benefit (mortality dominates >90%)")
            note_cell.font = Font(name="Arial", size=8, color="64748B", italic=True)
            ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=n+4)
            row += 1
        ...
        for scenario, col_key, color in [("Low",  "low",  C_RED),
            ...
            for mi in range(n):
                ...
                # Normalise sv_name for matching (strip trailing notes in parens)
                sv_key = sv_name.split("(")[0].strip().lower()
                ...
                elif any(k in sv_key for k in (
                    "vsl", "value of statistical life",
                    "heat mortality", "heat mortality factor",
                    "electricity tariff", "tariff",
                    "population", "vulnerable population",
                    "mortality rate", "hospitalization",
                    "benefit multiplier",
                )):
                    # All benefit-scaling parameters: multiplier × total annual benefit.
                    # Mortality dominates (>90% of benefit), so scaling total benefit
                    # is a good approximation for VSL, HMF, and population multipliers.
                    # For tariff/energy, it slightly overstates sensitivity but is directionally correct.
                    dr_use   = dr_inp
                    ben_term = f"({ben}*{param_ref})"
                else:
                    dr_use   = dr_inp
...
def _benefit_detail(wb, data, rm=None):
    ...
    def _build_avoided_mortality(comp, start_row):
        ...
        r_deaths = r
        deaths_cell = ws.cell(row=r, column=1,
            value="  🔍 Deaths Avoided / Year  ← REVIEW THIS NUMBER for realism")
        deaths_cell.font   = Font(name="Arial", size=9, bold=True, color="1E3A5F")
        deaths_cell.fill   = PatternFill("solid", fgColor="DBEAFE")
        deaths_cell.border = _bd()
        deaths_cell.alignment = Alignment(horizontal="left", vertical="center")

        deaths_val = ws.cell(row=r, column=2,
            value=f"=B{r_pop}*B{r_mrate}*B{r_hmf}*B{r_eff}")
        deaths_val.font         = Font(name="Arial", size=10, bold=True, color="1D4ED8")
        deaths_val.fill         = PatternFill("solid", fgColor="DBEAFE")
        deaths_val.border       = _bd()
        deaths_val.number_format = "0.0"
        deaths_val.alignment    = Alignment(horizontal="right")
    ...
    for m_idx, m in enumerate(measures):
        ...
        # ── Sanity-check warning: flag unrealistic BCR ──────────────────────
        # BCR > 15 for a climate adaptation measure almost always means a
        # parameter is wrong (VSL unit, population size, mortality rate).
        # Write a conditional warning row that appears automatically in Excel.
        C_WARN_BG = "FEF2F2"
        C_WARN_FG = "991B1B"
        ...
        warn_label.alignment = Alignment(wrap_text=True, vertical="center")
        ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=SPAN)
